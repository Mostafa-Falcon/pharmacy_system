...
class SyncCompactionService {
  ...
  Future<int> compactOutboxQueue() async {
    try {
      ...
      final Map<String, List<SyncOutboxTableData>> groups = {};
      for (final item in unsynced) {
        final key = '${item.targetTable}:${item.id}';
      ...
      for (final entry in groups.entries) {
        ...
        final firstOp = items.first.operationType.toLowerCase();
        final lastOp = items.last.operationType.toLowerCase();
        ...
        for (final item in items) {
          try {
            if (item.payloadJson.isNotEmpty) {
              final parsed = jsonDecode(item.payloadJson) as Map<String, dynamic>;
