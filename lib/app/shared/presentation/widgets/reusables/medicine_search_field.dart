...
class MedicineSearchField extends StatefulWidget {
  ...
  const MedicineSearchField({
    ...
    this.label = 'ابحث عن صنف أو باركود...',
    this.hint = 'ادخل اسم الدواء أو SKU...',
...
class _MedicineSearchFieldState extends State<MedicineSearchField> {
            ...
            child: Container(
              ...
              child: _results.isEmpty
                  ? Padding(
                      ...
                      child: const Center(
                        child: ReusableText('لا توجد نتائج مطابقة'),
                      ),
                    )
                  : ListView.separated(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  ReusableText(
                    'الباركود: ${medicine.barcodes.firstOrNull?.code ?? "---"} | الكمية: ${medicine.quantity}',
            ...
            Column(
              ...
              children: [
                ...
                if (medicine.buyPrice > 0)
                  ReusableText(
                    'شراء: ${FormatUtils.currency(medicine.buyPrice)}',
                    style: AppTextStyles.caption(context).copyWith(
                      color: isSelected
  ...
  void _onChanged(String value) {
    ...
    // طباعة بالكونسول للتأكد من نتائج البحث
    safeDebugPrint(
      'MedicineSearch: Searching "$query" in ${allItems.length} items',
    );

    final filteredResults = allItems
        .where((m) => !m.isDeleted && m.isActive)
        .where((m) {
          final nameMatch = m.name.toLowerCase().contains(query);
          final nameEnMatch = m.nameEn?.toLowerCase().contains(query) ?? false;
          final barcodeMatch = m.barcodes.any(
            (b) => b.code.toLowerCase().contains(query),
          );
    ...
    final exactMatch = _results.firstWhereOrNull(
      (m) => m.barcodes.any((b) => b.code.toLowerCase() == query),
    );
    if (exactMatch != null) {
      safeDebugPrint(
        'MedicineSearch: Exact barcode match found: ${exactMatch.name}',
      );
